...
# Protect admin endpoints (set a strong random value)
...
# Only set this locally if you want to use the admin dashboard in dev
VITE_ADMIN_KEY=replace-with-long-random-string
```
...
- Admin dashboard requests set header `x-admin-key` from `VITE_ADMIN_KEY` when present.
- Backend builds full short URLs using `PUBLIC_BASE_URL` if set; otherwise it defaults to `http://localhost:<PORT>`.

Visibility of Analytics UI:
- The Analytics section in the frontend is only shown when `VITE_ADMIN_KEY` is set in `Frontend/.env`.
- Backend access to `/admin/stats` and `/analytics/:shortId` still requires a valid `x-admin-key` that matches `ADMIN_API_KEY` on the server.
...
### GET /analytics/:shortId (protected)

Returns analytics for a specific short link.

```bash
curl -s http://localhost:8000/analytics/abc123 -H "x-admin-key: $ADMIN_API_KEY" | jq
```
...
Returns an array of recent URLs plus a summary. Protected with an API key header `x-admin-key`.

```bash
curl -s http://localhost:8000/admin/stats -H "x-admin-key: $ADMIN_API_KEY" | jq
```
...
- Expose only the endpoints you need; consider protecting `/admin/stats`
